...
class ditto {
	...
	function paginate($start, $stop, $total, $summarize, $tplPaginateNext, $tplPaginatePrevious, $tplPaginateNextOff, $tplPaginatePreviousOff, $tplPaginatePage, $tplPaginateCurrentPage, $paginateAlwaysShowLinks, $paginateSplitterCharacter) {
		...
		if ($stop == 0 || $total == 0 || $summarize==0 || ($total<=$summarize)) {
			return false;
		}
		...
		if($previous){$arg=$this->buildURL("start=$previous");}else{$arg=$modx->makeURL($modx->documentObject['id'],'');}
		$rPrevious =  $this->template->replace(array('url'=>$arg,'lang:previous'=>$ditto_lang['prev']),$tplPaginatePrevious);
		...
		for ($x = 0; $x <= $totalpages -1; $x++) {
			...
			$display = $x +1;
			if ($inc != $start) {
				if($inc){$arg=$this->buildURL("start=$inc");}else{$arg=$modx->makeURL($modx->documentObject['id'],'');}
				$pages .= $this->template->replace(array('url'=>$arg,'page'=>$display),$tplPaginatePage);
			} else {
