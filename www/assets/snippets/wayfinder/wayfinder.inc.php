...
class Wayfinder {
	...
	function getData() {
		...
		if (!empty($ids)) {
			...
			//add custom conditions to the where clause
			if($this->_config['where']) {
			$conds = explode('|', $this->_config['where']);
				foreach($conds as $cond){
					list($it,$that,$criterion) = explode(',', $cond);
					switch($criterion){
						case 1:
							$criterion = '!=';
							break;
					...
					$menuWhere .= ' AND sc.'.$it.$criterion.$that;
				}
			}
